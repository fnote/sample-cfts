  ...
  "Resources": {
	...
	"PriceConsoleLaunchConfig" : {
		...
		"Properties" : {
			...
			"UserData" : { "Fn::Base64" : { "Fn::Join" : ["", [
				...
				"#-----------------------------------\n",
				"sh -c \"echo 'export SERVER_ENVIRONMENT_VARIABLE=", { "Ref" : "EnvironmentShort" }, "'\" > /etc/profile.d/cpconsole.sh\n",
				"sh -c \"echo 'export CATALINA_OPTS=\\\"-Xms512M -Xmx1024M\\\"'\" >> /etc/profile.d/cpconsole.sh\n",
	...
	"lx238cpmcp01s" : {
		...
		"Properties" : {
			...
			"IamInstanceProfile" : { "Ref" : "InstanceProfileMCP" },
			"SubnetId": { "Ref": "PvtSNd" },
			"PrivateIpAddress" : "10.168.140.132",
			"BlockDeviceMappings" : [ {
				"DeviceName" : "/dev/sda1",
				"Ebs" : {
					"VolumeSize" : "60",
	...
	"lx238cpsync01s" : {
		...
		"Properties" : {
			...
			"UserData" : { "Fn::Base64" : { "Fn::Join" : ["", [
				...
				"# sh -c \"hostname  cpsync-$(curl http://169.254.169.254/latest/meta-data/local-ipv4/ -s)s.na.sysco.net\"\n",
				"#sh -c \"echo  cpsync-$(curl http://169.254.169.254/latest/meta-data/local-ipv4/ -s)s.na.sysco.net\" > /etc/hostname\n",
	...
	"lx238cpsyncpmt01s" : {
		...
		"Properties" : {
			...
			"Tags" : [
				{ "Key" : "Name", "Value" : "lx238cpsyncpmt01s" },
				{ "Key" : "Application_Name", "Value" : { "Ref" : "ApplicationName" } },
				{ "Key" : "Application_Id", "Value" : { "Ref" : "ApplicationId" } },
				{ "Key" : "Environment", "Value" : { "Ref" : "Environment" } },
				{ "Key" : "PO_Number", "Value" : { "Ref" : "PONumber" } },
			...
			"UserData" : { "Fn::Base64" : { "Fn::Join" : ["", [
				...
				"#Change hostname to include IP Address\n",
				"##############################################\n",
				"hostname lx238cpsyncpmt01s.na.sysco.net\n",
				"echo lx238cpsyncpmt01s.na.sysco.net > /etc/hostname","\n",
				"# sh -c \"hostname  cpsyncpmt-$(curl http://169.254.169.254/latest/meta-data/local-ipv4/ -s)s.na.sysco.net\"\n",
				"#sh -c \"echo  cpsyncpmt-$(curl http://169.254.169.254/latest/meta-data/local-ipv4/ -s)s.na.sysco.net\" > /etc/hostname\n",
	...
	"lx238cpsyncagm01s" : {
		...
		"Properties" : {
			...
			"Tags" : [
				{ "Key" : "Name", "Value" : "lx238cpsyncagm01s" },
				{ "Key" : "Application_Name", "Value" : { "Ref" : "ApplicationName" } },
				{ "Key" : "Application_Id", "Value" : { "Ref" : "ApplicationId" } },
				{ "Key" : "Environment", "Value" : { "Ref" : "Environment" } },
				{ "Key" : "PO_Number", "Value" : { "Ref" : "PONumber" } },
			...
			"UserData" : { "Fn::Base64" : { "Fn::Join" : ["", [
				...
				"#Change hostname to include IP Address\n",
				"##############################################\n",
				"hostname lx238cpsyncagm01s.na.sysco.net\n",
				"echo lx238cpsyncagm01s.na.sysco.net > /etc/hostname","\n",
				"# sh -c \"hostname  cpsyncagm-$(curl http://169.254.169.254/latest/meta-data/local-ipv4/ -s)s.na.sysco.net\"\n",
				"#sh -c \"echo  cpsyncagm-$(curl http://169.254.169.254/latest/meta-data/local-ipv4/ -s)s.na.sysco.net\" > /etc/hostname\n",
